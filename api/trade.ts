...
export default ({ request }: ClientContext) => ({
  ...
  orders: {
    create: (options: CreateOrderOptions) =>
      request<Order>({
        path: "/v2/orders",
        method: "POST",
        data: options,
      }),
    update: (options?: PatchOrderOptions | string, nested?: boolean) => {
      let path = "/v2/orders";
      let params = {};
  ...
  positions: {
    ...
    close: (params: ClosePositionOptions) =>
      request<Order[]>({
    ...
    exercise: (symbol_or_contract_id: string) =>
      request<void>({
  ...
  watchlists: {
    ...
    create: ({ name, symbols }: CreateWatchlistParams) =>
      request<Watchlist>({
        path: "/v2/watchlists",
        method: "POST",
        data: { name, symbols },
    ...
    delete: ({ watchlist_id }: { watchlist_id: string }) =>
      request<void>({
        path: `/v2/watchlists/${watchlist_id}`,
        method: "DELETE",
      }),
    ...
    removeSymbol: ({ watchlistId, symbol }: DeleteSymbolFromWatchlistParams) =>
      request<Watchlist>({
  ...
  configurations: {
    ...
    update: (updatedConfig: UpdatedAccountConfigurations) =>
      request<void>({
        path: "/v2/account/configurations",
        method: "PATCH",
        data: updatedConfig,
  ...
  assets: {
    ...
    // getAssetBySymbolOrId: (symbolOrAssetId: string) =>
    //   request<Asset>({
  ...
  crypto: {
    wallets: {
      ...
      whitelists: {
        ...
        request: (whitelistedAddressParams: WhitelistedAddressParams) =>
          request<WhitelistedAddress>({
            path: "/v2/wallets/whitelists",
            method: "POST",
    ...
    transfers: {
      ...
      withdraw: (withdrawalParams: WithdrawalParams) =>
