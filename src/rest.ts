...
export const trading = ({ request }: ClientContext) => ({
  v2: {
    account: {
      get: () =>
        request<unknown>({
...
export const data = ({ request }: ClientContext) => ({
  getCorporateActions: (queryParams: CorporateActionsQueryParams) =>
    request<CorporateActionsResponse>({
      path: "/v1beta1/corporate-actions",
      method: "GET",
      params: queryParams,
    }),
  getLatestForexRates: (currencyPairs: string) =>
  ...
  getLogos: (symbols: string, placeholder?: boolean) =>
    request<unknown>({
      path: "/v1beta1/logos",
      method: "GET",
      params: { symbols, placeholder },
    }),
});

export const meow = ({ request }: ClientContext) => ({
  v1beta1: {
    news: {
      get: (
        start?: string,
