...
Deno.test("Token bucket should allow taking tokens within capacity", () => {
...
Deno.test("Token bucket should reject taking tokens beyond capacity", () => {
  const tokenBucket = createTokenBucket({
    capacity: 200,
    fillRate: 3,
  });
...
Deno.test("Token bucket should refill tokens based on fill rate", async () => {
  const tokenBucket = createTokenBucket({
    capacity: 200,
    fillRate: 3,
  });
...
Deno.test(
  "Token bucket should support 200 requests per 60 seconds",
  async () => {
